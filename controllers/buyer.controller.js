...
export const getProductGroup = async (req, res) => {
  ...
  try {
    // Tìm nhóm sản phẩm và populate các tham chiếu liên quan
    const productGroup = await ProductGroupModel.findById(id)
      .populate('productIds') // Populate danh sách sản phẩm
      .populate('categoryIds'); // Populate danh sách danh mục

    if (!productGroup) {
      return res.status(404).json({ message: "Sản phẩm không tồn tại" });
    }

    // Đảm bảo image luôn là mảng
    const safeProductGroup = {
      ...productGroup.toObject(),
      image: Array.isArray(productGroup.image) ? productGroup.image : []
    };

    return res.status(200).json(safeProductGroup);
  } catch (error) {
